...
function showCalender(elPos, tgtEl)
{
	...
	if (document.getElementById(tgtEl.id))
	{
		targetEl = document.getElementById(tgtEl.id);
	}
	else
	{
		if (document.forms[0].elements[tgtEl.id])
		{
			targetEl = document.forms[0].elements[tgtEl.id];
		}
		else
		{
	...
	var matchDate = new RegExp('^([0-9]{2})-([0-9]{2})-([0-9]{4})$');
	 ...
	 var chosenDate = targetEl.value;
	 
	  if (chosenDate != ""){
	    ChosenDateArray = chosenDate.split(" ");
	    theMonth = ChosenDateArray[1];
	    for (j=0;j<monthnames.length;j++){
	        if (theMonth == monthnames[j]){
	            monthNum = j+1;
	            if (monthNum < 10){
	                chosenDate = ChosenDateArray[0] + "-0" + monthNum + "-" + ChosenDateArray[2];
	            }
	            else
	            {
	                chosenDate = ChosenDateArray[0] + "-" +  monthNum + "-" + ChosenDateArray[2];
	            }
	        }
	    }
	    
	  }
	var m = matchDate.exec(chosenDate);
	if (m == null)
	{
...
function pickDate(year, month, day)
{
	...
	{
		targetEl.value= day + ' ' + monthname[monthx-1] + ' '+year;
		closeCalender();
	}
}
